"""IQ3_S x bf16 GEMM in Gluon: the fourth int8 form on a tile-major repack
with 16-byte asynchronous copies.

The file's IQ3_S block is 110 bytes (d 2, qs 64, qh 8, signs 32, scales 4),
2-byte aligned, so the fourth form stages it with 4-byte cp.async (1,760 per
CTA and k-block, each an L1-access MIO instruction) and reads every field
through a two-word prmt straddle. The repack (repack_iq3s_tiles, once per
tensor on the host side) lays the weights out tile-major: for every (64-row
tile, k-block) a 7,168-byte tile of 64 rows x 112 bytes, each row the block's
fields word-aligned in the order the kernel reads them - qs (words 0-15), qh
(16-17), signs (18-25), scales (26), d (27, low half), two bytes of padding.
The kernel copies a tile with 448 16-byte cp.async (3.5 per thread, L1
bypassed), and every field is one ld.shared - no straddle selects, no
lane-variable shifts on the field path. Bytes per weight grow 1.8 % (110 to
112). The decode, the mma and the epilogue are the fourth form's: bit-identical.
...
# The grid table's offset in the kernel's dynamic shared memory (the fourth form's
# allocations and placement: the weight stage 16 KB, the A tile 8 KB, the table after
# them; found by a sweep, and a wrong one is caught by the perplexity identity;
# unchanged with the reduction's atomic).
TAB_OFF = 24576

# the two shared-memory helpers of the sixth bf16 form, inlined (as in iq3s_int8c.py)
@g.jit
def _smem_base(like):
...
def repack_iq3s_tiles(W: torch.Tensor, n_out: int) -> torch.Tensor:
    """Raw IQ3_S rows [n_out, nb * 110] (uint8) -> tile-major uint8
    [n_tiles, nb, 64, 112]: the fields word-aligned in the kernel's order, d
    last, rows past n_out zero."""
    assert W.dtype == torch.uint8 and W.dim() == 2 and W.shape[0] == n_out and W.shape[1] % BLOCK_BYTES == 0
    nb = W.shape[1] // BLOCK_BYTES
    n_tiles = -(-n_out // 64)
    blk = W.reshape(n_out, nb, BLOCK_BYTES)
    d, qs, qh, sg, sc = blk[..., 0:2], blk[..., 2:66], blk[..., 66:74], blk[..., 74:106], blk[..., 106:110]
    pad = torch.zeros((n_out, nb, 2), dtype=torch.uint8, device=W.device)
    t = torch.cat([qs, qh, sg, sc, d, pad], dim=-1)              # [n_out, nb, 112]
    if n_tiles * 64 > n_out:
        t = torch.cat([t, torch.zeros((n_tiles * 64 - n_out, nb, TILE_ROW_BYTES), dtype=torch.uint8, device=W.device)], dim=0)
    return t.reshape(n_tiles, 64, nb, TILE_ROW_BYTES).permute(0, 2, 1, 3).contiguous()
...
@g.jit
def iq3s_int8t_kernel(XQ, SX, WT, Y, GRID32,
...
def iq3s_int8t_gemm(WT: torch.Tensor, X: torch.Tensor, n_out: int, splitk: int | None = None, num_warps: int = 4,
    ...
    iq3s_int8t_kernel[(WT.shape[0], splitk)](
