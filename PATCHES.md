...
| int4-attn-int8qk-bf16pv | fix | int4 attention on tensor cores: Q int8 per row and K int8 (int32 dot) instead of fp32/TF32, P and V bf16, BLOCK_M 64 (one query block per request), decode tile 64, prefill tile 128, 8 warps | none | 0.29.0 | rides with int4-kv-per-token-head |
| int4-prefill-dequant-fa2 | feature | int4 prefill chunks: dequantize the request's cached K/V per layer into a bf16 workspace and run FlashAttention-2 varlen on it; decode and verify batches stay on the packed kernel | none | 0.29.0 | rides with int4-kv-per-token-head |
...
| sched-spec-full-width | fix | a speculative decode row is scheduled at its full width or waits for the next step, so the GDN kernels' accepted-token state offsets never meet a narrower step (replaces gdn-active-runtime-k-width) | none yet | 0.29.0 + the series above | upstream PR |
...
| `gguf-plugin/install.sh` | feature | the out-of-tree GGUF plugin at a pinned commit, eleven patches, twenty-six Gluon decode kernels (eleven tile types on the grouped one) | three hash gates (`gguf-plugin/README.md`); `verify.sh` checks the package, its extension and its model test |
...
Last in `patches/series`. Of the series above only `dflash2-backport` touches `config/speculative.py`, and the
install skips it; `qwen3_5_mtp.py` is last touched by `qwen3_5-mtp-draft-vocab`.

| patch | kind | what | upstream | cut against | retires when |
|---|---|---|---|---|---|
| spec-mtp-draft-from-target | fix | method `mtp` without a draft model of its own: the draft `ModelConfig` takes the target's `model`, `model_weights` and `hf_config_path`, so a GGUF target (config directory and `.gguf` file) boots with its MTP layer as the drafter | none yet | 0.29.0 | upstream takes it |
| qwen3_5-mtp-skip-replaced-vocab | fix | the Qwen3.5 MTP draft's `lm_head`, and without pipeline parallelism its `embed_tokens`, which both proposer paths replace with the target's after loading, are built on the meta device and their checkpoint tensors skipped (2 x 2.37 GiB on Qwen3.8-27B); a head tied to the embedding or untied by the checkpoint keeps the previous construction | none yet | 0.29.0 + the series above | upstream takes it |
